...
The GraphQL schema defines the root object type Query.  The compiler appends the suffix `Service` to
this root object type name to derive the Java class name QueryService.  As an application developer,
you must implement the QueryService class.  By convention, this class is in the Java package
configured by the _packagePrefix_ property of the Maven plugin.
